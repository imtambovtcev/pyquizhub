    ...
    def _validate_constraints(
        ...
        if var_type == VariableType.STRING:
            if constraints.min_length is not None and len(
                ...
                value_len = len(value)
                raise ValueError(
                    f"Variable '{name}' length {value_len} below minimum {
                        constraints.min_length}")
            if constraints.max_length is not None and len(
                    value) > constraints.max_length:
                value_len = len(value)
                raise ValueError(
                    f"Variable '{name}' length {value_len} above maximum {
