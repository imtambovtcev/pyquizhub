    ...
    def eval_expr(expr: str, variables: dict) -> any:
        ...
        def _eval(node):
            if isinstance(node, ast.BinOp):
                ...
                logger.debug(f"Attribute access: value={value}, attr={node.attr}, value_type={type(value)}")
                if isinstance(value, dict) and node.attr in value:
                    return value[node.attr]
                keys_str = value.keys() if isinstance(value, dict) else 'N/A'
                logger.error(f"Attribute access failed: value={value}, attr={node.attr}, keys={keys_str}")
