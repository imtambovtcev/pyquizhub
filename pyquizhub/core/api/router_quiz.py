...
def submit_answer(quiz_id: str, request: AnswerRequestModel, req: Request):
    ...
    if session_data["quiz_id"] != quiz_id:
        logger.error(f"Quiz ID mismatch: expected {session_data['quiz_id']}, got {quiz_id}")
    ...
    if new_engine_state["completed"]:
        ...
        logger.info(f"Completed quiz session {session_id} for user {user_id} on quiz {quiz_id}")
